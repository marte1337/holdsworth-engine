...
// The 922, 9.13 CONNECT, and GROUP diagnostics remain available as DSP presets
// but are intentionally absent from this milestone's live selector.
  ...
  const std::uint32_t index) noexcept
{
  switch (index)
  {
    ...
    case static_cast<std::uint32_t>(DevelopmentDelayPreset::holdsworth122):
      return DevelopmentDelayPreset::holdsworth122;
  ...
  const DevelopmentDelayPreset preset) noexcept
{
  switch (preset)
  {
    case DevelopmentDelayPreset::holdsworth122:
      return dsp::presets::holdsworth122ProvisionalV1();
